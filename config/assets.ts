...
export interface AssetTemplate {
  ...
  // Whether this asset's generated text is a script that should also
  // trigger a real HeyGen avatar video render (see lib/heygen/generate.ts).
  // The text remains the asset's primary content either way -- video
...
12. The Ask / CTA -- capital raise objective, investor fit, next steps, data room invitation, AND a closing contact block: the fund's primary contact name, title, email, and phone number, each pulled only from the knowledge base's own General Information fields (Full Name, Current Title & Role, Email, Phone Number). Include only the specific fields actually present -- if a field is missing, simply omit that line rather than guessing or writing a placeholder like "[phone]" or "N/A". If NONE of those contact fields are present in the knowledge base, end the slide with "Contact details available upon request" instead of a contact block.

OUTPUT FORMAT -- CRITICAL
Output ONLY Markdown, nothing before or after it, no commentary. One "## " heading per slide, in the exact order above, using the slide name as the heading (e.g. "## The Problem"). Under each heading, write 3-5 short bullet points (each under ~15 words -- these become on-slide text, so presentation-length, not paragraph-length) using "- " bullets -- except slide 12, which gets its usual bullets plus the contact block described above as its own short bullet or two at the end (e.g. "- Contact: Jane Doe, Managing Partner" / "- jane@fund.com · (555) 123-4567"). After the bullets for slides 2-12, add one short paragraph labeled "Speaker notes:" with 1-3 sentences of additional context or talking points -- this is where nuance, caveats, or data-point detail goes, since it won't appear on the visible slide. The cover slide (## Cover) needs only a one-line subtitle, no bullets or notes. Do not include a title above the first "## Cover" heading, and do not include any text outside this structure.` +
      KB_GUARDRAILS,
    buildUserPrompt: (a) =>
      withKnowledgeBase(
...
Clear section headers. Output only the finished framework -- no preamble beyond the note above.` +
  ...
  },
  {
    ...
    phase: "3c",
    outputFormat: "docx",
    supportsHeygenVideo: true,
    systemPrompt:
      `You are writing the spoken script and production brief for an AI-generated interview-style video (via HeyGen) featuring the fund's principal.

Do not invent statistics, deals, or claims not present in the knowledge base.

STRUCTURE
1. Video Concept -- one-line description of the video's angle/purpose
2. Interview Q&A Script -- 4-6 interviewer questions with the principal's suggested talking-point answers, natural spoken register (for the human team's reference -- shows the intended back-and-forth)
...
5. Avatar Script -- REQUIRED, wrapped exactly between the literal markers <<<AVATAR_SCRIPT>>> and <<<END_AVATAR_SCRIPT>>> on their own lines. This is a single continuous first-person monologue (60-120 seconds spoken) that covers the same talking points as the Q&A above, but reworked into one voice speaking straight through -- no "Q:"/"A:" labels, no interviewer lines, no bracketed directions, no headers inside the markers. Every word between the markers gets performed verbatim by an AI avatar, so it must be complete, natural spoken sentences and nothing else.

OUTPUT FORMAT
Clear section headers for 1-4. Section 5 uses the exact markers described above. Output only the finished script and brief -- no preamble.` +
      KB_GUARDRAILS,
    buildUserPrompt: (a) =>
  ...
  },
  {
    ...
    supportsHeygenVideo: true,
    systemPrompt:
      `You are producing a selection brief for short-form video clips that would be cut from a longer source video, PLUS a short standalone avatar-spoken video that presents this fund's clip-worthy talking points directly (used when no source footage exists yet to clip from).
...
2. Suggested Clip Themes -- 4-5 types of moments worth clipping if source footage exists (e.g. "the fund's answer to the track-record objection," "a specific differentiator explained in one breath")
...
5. Avatar Script -- REQUIRED, wrapped exactly between the literal markers <<<AVATAR_SCRIPT>>> and <<<END_AVATAR_SCRIPT>>> on their own lines. A single continuous first-person monologue (30-45 seconds spoken) delivering ONE of the clip-worthy talking points from section 2, in the fund principal's voice, as complete natural spoken sentences -- no headers, labels, or bracketed directions inside the markers. Every word between the markers gets performed verbatim by an AI avatar.

OUTPUT FORMAT
Clear section headers for 1-4. Section 5 uses the exact markers described above. Output only the finished brief -- no preamble.` +
      KB_GUARDRAILS,
    buildUserPrompt: (a) =>
  ...
  },
  {
    ...
    supportsHeygenVideo: true,
    systemPrompt:
      `You are producing a script and production brief for a short vertical-format Reel/short-form video.

Do not invent statistics, deals, or claims not present in the knowledge base.

STRUCTURE
1. Hook (first 2-3 seconds) -- the strongest single line to open on
2. Beat-by-Beat Script -- short spoken/on-screen lines broken into beats, with bracketed visual suggestions between them (for the human production team's reference)
...
6. Avatar Script -- REQUIRED, wrapped exactly between the literal markers <<<AVATAR_SCRIPT>>> and <<<END_AVATAR_SCRIPT>>> on their own lines. The spoken lines from the Beat-by-Beat Script above, combined into one continuous monologue with the bracketed visual directions removed -- just the words an avatar would actually say, nothing else, as complete natural spoken sentences.

OUTPUT FORMAT
Clear section headers for 1-5. Section 6 uses the exact markers described above. Output only the finished script and brief -- no preamble.` +
      KB_GUARDRAILS,
    buildUserPrompt: (a) =>
