...
## Full asset catalog + Prompt Library

**All planned assets now exist** — 53 total (6 foundational + 47 marketing across Phases 2, 3A, 3B, 3C). Phase 3B (Sales Enablement) is straightforward text. Phase 3C (Video & Audio) has one honesty caveat:

- Transcript Repurposing, Podcast Preparation, Book Outline, Interview Questions — real, fully-working text assets.
- **AI Interview Video, Short Clips, Reels** — no video/clipping API is connected yet, so these three generate a **text script and production brief** instead of actual video, clearly labeled as a placeholder at the top of the output ("no video is generated by this system yet"). Once HeyGen/Opus Clip/etc. are wired up, these can be upgraded the same way images and pptx decks were — auto-generated media alongside the text, not a separate manual step.

**Prompt Library** (`/dashboard/prompt-library`) — every asset's system prompt is now editable from the UI, reversing the earlier "code + redeploy" decision now that the catalog is large enough to need it:

- **General override** — applies to every client's projects for that asset, unless a client-specific override exists.
- **Client-specific override** — takes priority over the general override, scoped to one client's projects only.
- Resolution order at generation time: client override → general override → hardcoded default in `config/assets.ts`. If neither override exists, nothing changes from before.
- Editing a prompt **never removes the anti-fabrication guardrails** — those get appended automatically at generation time regardless of whether you're using the default or a custom override, so an override can't accidentally (or deliberately) drop them.
- Switch scope via the dropdown at the top (General vs. a specific client); click **Edit** on any asset to see/change the effective prompt for that scope, or **Reset to default** to remove an override entirely.
- Only the **system prompt** (the instructions) is editable — the knowledge-base-assembly logic that injects the fund's actual onboarding answers is unaffected and still runs automatically underneath whatever prompt is active.
